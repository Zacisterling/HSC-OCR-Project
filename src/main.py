...
def load_font_dataset():
    ...
    image_dir = "../FONTS_comprehensive_character_images"
...
def load_handwritten_dataset():
    ...
    preprocessor = ImagePreprocessor(target_size=(28, 28))
    image_dir = "../HANDWRITTEN_emnist_character_images"
    
    if not os.path.exists(image_dir):
        print(f"❌ Handwritten dataset not found: {image_dir}")
        print("💡 Run HANDWRITTEN_emnist_integration.py first!")
        return None, None
...
def train_handwritten_model():
    ...
    # Train using existing training method
    ocr_model.train_model(X_train, y_train, X_test, y_test, epochs=10, batch_size=32)
